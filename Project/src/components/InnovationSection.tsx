...
import { Lightbulb, Star, GitFork, ExternalLink, Loader2, RefreshCw } from 'lucide-react';
import { Link } from 'react-router-dom';
import { fetchInnovationRepos } from '../utils/githubApi';
...
export function InnovationSection() {
  ...
  const loadInnovationRepos = async (force: boolean = false) => {
    try {
      setLoading(true);
      const innovationRepos = await fetchInnovationRepos(force);
      setRepos(innovationRepos.slice(0, 4));
      ...
      setError('Failed to load innovation repositories');
    } finally {
      setLoading(false);
    }
  };
    ...
    <div className="w-full max-w-4xl">
      <div className="flex items-center justify-between mb-4">
        <div className="flex items-center gap-2">
          ...
          <h2 className="text-2xl font-bold text-[var(--text-primary)]">Innovation Hub</h2>
        </div>
