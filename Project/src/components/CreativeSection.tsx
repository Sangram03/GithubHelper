...
import { Palette, Star, GitFork, ExternalLink, Loader2, RefreshCw } from 'lucide-react';
import { Link } from 'react-router-dom';
import { fetchCreativeRepos } from '../utils/githubApi';
...
export function CreativeSection() {
  ...
  const loadCreativeRepos = async (force: boolean = false) => {
    try {
      setLoading(true);
      const creativeRepos = await fetchCreativeRepos(force);
      setRepos(creativeRepos.slice(0, 4));
      ...
      setError('Failed to load creative repositories');
    } finally {
      setLoading(false);
    }
  };
    ...
    <div className="w-full max-w-4xl">
      <div className="flex items-center justify-between mb-4">
        <div className="flex items-center gap-2">
          ...
          <h2 className="text-2xl font-bold text-[var(--text-primary)]">Creative Hub</h2>
        </div>
